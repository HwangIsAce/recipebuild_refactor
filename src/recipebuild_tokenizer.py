...
class RBTokenizer:
    ...
    def __str__(self) -> str:
        return f"Custom Tokenizer = {self.rb_config['tokenizer']}"
    ...
    def train(self):
        ...
        _train_file_path = self.rb_data["for_train_tokenizer_dir"]

        # load dataset to train the tokenizer
        self.rb_ds = load_dataset("text", data_files={"train":_train_file_path})
        
        special_tokens = self.rb_config['special_tokens']
        
        trainer = trainers.WordPieceTrainer(vocab_size=self.rb_config["vocab_max_size"], special_tokens=special_tokens)
    ...
    def save(self):
        ...
        self._tokenizer.save(self.rb_config["tokenizer"])
        
    def load(self):
        self.tokenizer = PreTrainedTokenizerFast(
            tokenizer_file=self.rb_config["tokenizer"], # You can load from the tokenizer file, alternatively
            unk_token="[UNK]",
            pad_token="[PAD]",
            cls_token="[CLS]",
            sep_token="[SEP]",
